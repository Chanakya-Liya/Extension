...
class SimpleTreeDataProvider {
    ...
    getChildren(element) {
        if (element) {
            ...
            const welcomeItem = new TreeItem('Enter your Bugs and Queries here to get AI suggestions', vscode.TreeItemCollapsibleState.None);
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(AiChatViewProvider.viewType, provider));
    ...
    const pickWorkspaceFileCommand = vscode.commands.registerCommand('Ai-Chat.pickWorkspaceFile', async () => {
        ...
        for (const folder of workspaceFolders) {
            try {
                ...
                const files = await vscode.workspace.findFiles('**/*', // Include all files
...
class AiChatViewProvider {
    ...
    addAttachment(content, fileName, filePath) {
        if (this._view) {
            this._view.webview.postMessage({
    ...
    async _analyzeCode(code, purpose, attachments = []) {
        try {
            ...
            outputChannel.appendLine('==================================================');
            // Send the request
            const response = await (0, node_fetch_1.default)('http://127.0.0.1:8080/analyze', {
                method: 'POST',
                headers: {
